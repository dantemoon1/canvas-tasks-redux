...
export function watchCustomColors(
  ...
  const listener = (changes: {
    ...
    if (key in changes) {
      Object.entries(changes[key].newValue as Record<string, string>).forEach(
        (entry: [string, string]) => {
          const oldVal = (changes[key].oldValue || {}) as Record<string, string>;
          if (entry[0] in oldVal && oldVal[entry[0]] == entry[1])
            return;
